...
--3. Revenue by Customer Status

select Customer_Status,count(customer_status) as total_count, sum(Total_Revenue) as total_revenue,
cast(sum(total_revenue) * 100.0/(select sum(total_revenue) from stg_Churn) as decimal(10,2)) as revenue_percentage
from stg_Churn
...
select state, count(customer_id) as total_count,
cast(count(customer_id) * 100.0/(select count(*) from stg_Churn) as decimal(10,2)) as percentage
from stg_Churn
group by state
